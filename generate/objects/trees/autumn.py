...
LEAF_COLOR_MIN = (40, 20, 10, 90)
LEAF_COLOR_MAX = (150, 80, 20, 110)
...
def drawBranchRecursive(startPoint, diameter, angle, resolution, branchDraw : ImageDraw.ImageDraw, leafBackgroundDraw : ImageDraw.ImageDraw, leafForegroundDraw : ImageDraw.ImageDraw, branchColor=BRANCH_COLOR, leafColor=LEAF_COLOR):
    ...
    if random.random() < SPLIT_RATIO:
        ...
        drawBranchRecursive(startPoint, diameterA, angleA, resolution, branchDraw, leafBackgroundDraw, leafForegroundDraw, branchColor, leafColor)
        drawBranchRecursive(startPoint, diameterB, angleB, resolution, branchDraw, leafBackgroundDraw, leafForegroundDraw, branchColor, leafColor)
    else:
        ...
        if nextDiameter >= MIN_DIAMETER:
            drawBranchRecursive(nextPoint, nextDiameter, nextAngle, resolution, branchDraw, leafBackgroundDraw, leafForegroundDraw, branchColor, leafColor)
        else:
            if (startPoint[1] <= MAX_LEAF_Y):
                modifiedLeafColor = getRandomColorDeviation(leafColor, 5)
...
for i in range(numToDraw):
    ...
    leafStartColor = getRandomColorFromRange(LEAF_COLOR_MIN, LEAF_COLOR_MAX)
    drawBranchRecursive(START, 20, radians(-90), 3, branchDraw, leafBackgroundDraw, leafForegroundDraw, leafColor=leafStartColor)
    ...
    filename = OUTPUT_PATH + "autumn_" + str(i) + ".png"
